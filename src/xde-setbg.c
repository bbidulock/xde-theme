...
xdeSetProperties(SmcConn smcConn, SmPointer data)
{
	...
	prop[j].num_vals = 1;
	props[j] = &prop[j];
	propval[j].value = "xde-setbg -q";
	propval[j].length = strlen("xde-setbg -q");
	j++;
#endif
...
main(int argc, char *argv[])
{
	...
	while (1) {
		...
		static struct option long_options[] = {
			{"run",		no_argument,		NULL, '2'},
			{"quit",	no_argument,		NULL, 'q'},
			{"restart",	no_argument,		NULL, 'r'},
			{"recheck",	no_argument,		NULL, 'c'},
			{"edit",	no_argument,		NULL, 'e'},
			{"set",		required_argument,	NULL, '3'},
		...
		c = getopt_long_only(argc, argv, "qrcRbnlad:w:gsmt:AD::v::hVCH?", long_options,
				     &option_index);
#else				/* defined _GNU_SOURCE */
		c = getopt(argc, argv, "qrcRbnlad:w:gsmt:ADvhVC?");
