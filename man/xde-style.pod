...
long options with a single dash are accepted.  B<xde-style> accepts the
following options:
...
Not all window managers can distinguish between system and user styles.
Case in point is L<icewm(1)>, which, when setting a style, will use the
first style encountered.  When a style name is both a user a system
style, the user style takes precedence.  This option has no effect on
L<icewm(1)>.
...
environment varaiable, in which case the screen number defaults to the
...
window manager looks for auxilliary files (such as pixmaps) in the same
...
When setting the style, as the window manager to reload the new style.
...
Example:
 ...
 Squared-blue /usr/share/waimea/styles/Squared-blue
